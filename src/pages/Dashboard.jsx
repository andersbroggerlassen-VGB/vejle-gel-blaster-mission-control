...
export default function Dashboard({ state }) {
  return <Layout title="MISSION CONTROL SYSTEM">
    <div className="grid dashboard-grid">
      <Panel title="MISSION BRIEFING" className="brief"><h1>{state.missionName}</h1><h3>{state.missionType}</h3><p>{state.missionText}</p><ul>{state.rules?.map((r,i)=><li key={i}>{r}</li>)}</ul><div className="countdown">{formatTime(state.remaining)}</div></Panel>
