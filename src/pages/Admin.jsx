...
export default function Admin({ state }) {
  ...
  const start = () => {
    const seconds = Number(state.remaining || state.duration || 900);
    patchState({
      status: 'LIVE',
      remaining: seconds,
  ...
  const pauseResume = () => {
    if (state.status === 'LIVE') {
      const secondsLeft = getRemainingSeconds(state);
      return patchState({ status: 'PAUSED', remaining: secondsLeft, endsAt: null });
    }
    if (state.status === 'PAUSED') {
      const seconds = Number(state.remaining || state.duration || 900);
      return patchState({ status: 'LIVE', remaining: seconds, endsAt: Date.now() + seconds * 1000 });
    }
  };
    ...
    <div className="admin-grid">
      <Panel title="KAMPSTYRING">
        <div className="buttons">
          ...
          <button onClick={pauseResume}>{state.status === 'PAUSED' ? 'GENOPTAG' : 'PAUSE'}</button>
          <button className="danger" onClick={stop}>STOP / NULSTIL</button>
        </div>
        <div className="buttons">
          <button className="red" onClick={()=>patchState({redScore: Number(state.redScore)+1, soundEvent:{type:'point',at:Date.now()}})}>+1 RØD</button>
          <button className="blue" onClick={()=>patchState({blueScore: Number(state.blueScore)+1, soundEvent:{type:'point',at:Date.now()}})}>+1 BLÅ</button>
          <button onClick={()=>patchState({redScore: Math.max(0, Number(state.redScore)-1)})}>-1 RØD</button>
          <button onClick={()=>patchState({blueScore: Math.max(0, Number(state.blueScore)-1)})}>-1 BLÅ</button>
        </div>
        <label>Missionstid i minutter
          <input type="number" value={Math.round(Number(state.duration)/60)} onChange={e=>{
            const d=Number(e.target.value)*60;
            patchState({duration:d, remaining:d, status:'READY', endsAt:null});
          }} />
        </label>
      </Panel>
